...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, LPWSTR cmdLine, INT)
{
    ...
    for (int n = 0; n < argc; ++n)
    {
        ...
        {
            MessageBoxA(NULL, szUsage, "WinDosDX Crash Reporter", MB_OK);
    ...
    if (!PathIsDirectoryW(OutputPath.c_str()))
    {
        ...
        if (res != ERROR_SUCCESS && res != ERROR_ALREADY_EXISTS)
        {
            ...
            MessageBoxA(NULL, "Could not create directory to write crash report.", "WinDosDX Crash Reporter", MB_ICONERROR | MB_OK);
            return abort(stdout, 0);
        }
    }
