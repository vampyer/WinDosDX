...
static void W32_ConfDir(std::string& in,bool create) {
	...
	BOOL r = FALSE;
	/* WinDosDX keeps DOSBox configuration inside the WinDosDX application
	 * data tree; avoid importing the optional shell32 SHGetSpecialFolderPath
	 * helper into the internal machine. */
	if (create)
	{
		char const * windir = getenv("windir");
		if (!windir) windir = "c:\\windows";
		safe_strncpy(result, windir, MAX_PATH);
		char const* appdata = "\\WinDosDX";
		size_t len = strlen(result);
		if (len + strlen(appdata) < MAX_PATH) strcat(result, appdata);
		mkdir(result);
	}
	(void)r;
	in = result[0] ? result : "c:\\windows\\WinDosDX";
}
#endif
