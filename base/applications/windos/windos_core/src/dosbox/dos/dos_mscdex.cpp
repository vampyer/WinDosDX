...
int CMscdex::AddDrive(Bit16u _drive, char* physicalPath, Bit8u& subUnit)
{
	...
	case 0x00: {	
...
#if defined (WINDOSDX_PHYSICAL_CDROM)
		// Check OS
		OSVERSIONINFO osi;
		...
		LOG(LOG_MISC,LOG_NORMAL)("MSCDEX: IOCTL Interface.");
#else
		/* WinDosDX's physical CD-ROM backend is intentionally internal and
		 * currently has no host drive.  Keep the DOSBox image/fake paths
		 * available without pulling in an external frontend. */
