...
static Bitu Normal_Loop(void) {
	Bits ret;
	while (1) {
		/* Pump Win32 messages even while the emulated CPU has a full PIC
		 * queue.  The old placement only pumped on idle slices, so a DOS
		 * shell waiting on BIOS keyboard input could starve the host window
		 * message queue and appear frozen. */
		GFX_Events();
		if (PIC_RunQueue()) {
			ret = (*cpudecoder)();
